...
#define BUTTON_IS_PRESSED      (!((PINC >> PINC1) & 1))      // Pin 4 of port C (Button) is pressed. when button is pushed, PINC4 is pulled LOW.
...
const float V_f = 4.8;               //V/f value. ~230[VAC] w/ 60[Hz]
const float VBus = 240.0;                //AC voltage [VAC]
...
// First divide by 16 then multiply by 128 
int16_t Sine_Used[] = {125,179,223,246,246,223,179,125,71,27,6,6,27,71,125};
uint8_t  Click_Type = 0;                  //1: Short, 2: Long
uint8_t  PWM_Running = PWM_NOT_RUNNING;       //Indicates if the PWM is operating or not. 2 is running, 1 is not, initialized to 0 to indicate not yet set.
...
{lcd.init();                      // initialize the lcd 
  ...
  PORTC = (1 << PORTC3) | (1 << PORTC1);      //Activates internal pull up for PC3 (ADC3) and PC4 (ADC4). Default pin state is input. Potentiometer switch and button respectively   
  ...
  DDRB = (1 << DDB3) | (1 << DDB2) | (1 << DDB1) | (1 << DDB0); //Sets the OC2A, OC1B and OC1A pins to outputs (Default is LOW)
   lcd.setCursor(3,0);
  lcd.print("#STARTING!") ;           //And sets PB0 pin to output (Default is LOW). Commands the relay
...
void Pot_Switch_State_Check()    
{
   ...
   {  
    if (Timer - Timer_Temp  > 1) Pot_Switch_On_Timer = 0;    //To make sure these increments are consecutive
        ...
        if (Pot_Switch_On_Timer > 500)      //If potentiometer switch was ON a sufficient amount of time
        {
          state = 1;
     ...
     if (PWM_Running != PWM_NOT_RUNNING)     //If PWM is running (If it was already not running, no need to update anything).
      {
        ...
        Timer_Temp = Timer;
        Pot_Switch_On_Timer = 0;
        if (Pot_Switch_Off_Timer >  100)     //If potentiometer switch was OFF a sufficient amount of time
        {
          state = 0;
          Pot_Switch_On_Timer = 0;
          Pot_Switch_Off_Timer = 0;
...
void Display(uint8_t PWM_Running, bool Blink,uint8_t Desired_Freq)
{
 ...
 {
  Display_Timer++;
 if (Display_Timer > (2*DISPLAY_BLINK))
   lcd.clear();
    lcd.setCursor(3,0);
    lcd.print("FREQ:");
   
  lcd.print(Desired_Freq) ; 
   lcd.print("Hz");
   Display_Timer = 0;
  }
  else
...
{   
   ...
   if (OVF_Counter >= OVF_Counter_Compare)
   {
      ...
      {   if (Phase_Config == ONE_PH){OCR2A = 0;}
         else OCR1A = 0;
      }
      else
      {if (Phase_Config == ONE_PH){OCR2A = uint8_t(Sine_Used[Sine_Index_120] - DEATTIME_SUB); }
         else OCR1A = uint8_t(Sine_Used[Sine_Index_120] - DEATTIME_SUB);    
      }
      if (Phase_Config == ONE_PH){ OCR2B = OCR2A + DEATTIME_ADD;}
       else  OCR1B = OCR1A + DEATTIME_ADD;

      if ((Sine_Used[Sine_Index_240] - DEATTIME_SUB) < MIN_PWM_VAL)
      {if (Phase_Config == ONE_PH){OCR1A = 0;}
         else  OCR2A = 0;
      }            
      else
      {  if (Phase_Config == ONE_PH){  OCR1A = uint8_t(Sine_Used[Sine_Index_240] - DEATTIME_SUB);}
          else OCR2A = uint8_t(Sine_Used[Sine_Index_240] - DEATTIME_SUB);
      }
      if (Phase_Config == ONE_PH){  OCR1B = OCR1A + DEATTIME_ADD;}
      else OCR2B = OCR2A + DEATTIME_ADD;
